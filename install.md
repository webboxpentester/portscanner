...
echo "" && echo "" && echo -n "Which environment are you on? (1=Termux / 2=Linux) Type (1/2): " && read choice && if [ "$choice" -eq 1 ]; then sed -i 's|#for_settings_up 10101010|#!/data/data/com.termux/files/usr/bin/bash|' x.sh && sed -i 's|#for_settings_up 10101010|#!/data/data/com.termux/files/usr/bin/bash|' dependency_if_need.sh && echo "Updated for Termux in x.sh and dependency_if_need.sh"; elif [ "$choice" -eq 2 ]; then sed -i 's|#for_settings_up 10101010|#!/bin/bash|' x.sh && sed -i 's|#for_settings_up 10101010|#!/bin/bash|' dependency_if_need.sh && echo "Updated for Linux in x.sh and dependency_if_need.sh"; else echo "Invalid choice. Please run again and type 1 or 2."; fi  && chmod +x autosqli.sh && chmod +x dependency_if_need.sh
